...
def show_all(df, n=5):
    ...
    df_describe = (
        ...
        .set_caption("Describe Data Num")
    )
    df_describe_o = (
        df.describe(include=[object])
        .style.set_table_attributes("style='display:inline'")
        .set_caption("Describe Data Object")
    )
    df_head = (
        df.head(n).style.set_table_attributes("style='display:inline'").set_caption("Head Data")
    )
    df_tail = (
        df.tail(n).style.set_table_attributes("style='display:inline'").set_caption("Tail Data")
    )
    df_missing = (
        missing_values(df)
        .style.set_table_attributes("style='display:inline'")
        .set_caption("Missing Value")
    )
    df_frequent = (
        most_frequent_values(df)
        .style.set_table_attributes("style='display:inline'")
        .set_caption("Frequent Value")
    )
    df_unique = (
        unique_values(df)
        .style.set_table_attributes("style='display:inline'")
        .set_caption("Unique Value")
    )
...
def sweetviz_report(
    ...
    my_report = sv.compare(
        [train, "Training Data"], [test, "Test Data"], target_col, feature_config
    )
...
def plot_importance(
    ...
    | List[xgboost.core.Booster]
    | List[catboost.core.CatBoostRegressor]
    | List[catboost.core.CatBoostClassifier],
    ...
    for i, model in enumerate(models):
        ...
        elif isinstance(model, catboost.core.CatBoostRegressor) or isinstance(
